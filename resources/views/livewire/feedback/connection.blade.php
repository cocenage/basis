    ...
    @error('phone') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        ...
        <div class="w-full ">
            <textarea
                ...
                rows="4"
                class="w-full px-6 py-4 border-2 text-black rounded-[15px] focus:outline-none  "
            ></textarea>
            @error('message') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>


    <button   type="submit" class="w-max text-[16px] cursor-pointer px-[24px] mt-[30px] py-[12px] rounded-[10px] bg-[#f2f2f2] hover:bg-[#e1e1e1] duration-300">
           Отправить
        </button>
